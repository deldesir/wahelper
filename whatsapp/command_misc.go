...
func (c *Client) handleAppStateCommand(args []string) error {
	if len(args) == 0 {
		c.Logger.Errorf("Usage: appstate <names...>")
		return nil
	}
...
func (c *Client) handleUnavailableRequestCommand(args []string) error {
	...
	resp, err := c.WAClient.SendMessage(
		...
		"",
		c.WAClient.BuildUnavailableMessageRequest(chat, sender, args[2]),
...
func (c *Client) handleChatPresenceCommand(args []string) error {
	...
	err := c.WAClient.SendChatPresence(jid, types.ChatPresence(args[1]), types.ChatPresenceMedia(args[2]))
	if err != nil {
...
func (c *Client) handleMuteCommand(args []string) error {
	...
	if len(args) < 3 {
		...
		if t == 0 {
			hours = 318538 * time.Hour // Indefinite mute
		} else if t > 0 && t <= 168 {
			hours = time.Duration(t) * time.Hour
		} else {
			hours = 8 * time.Hour
		}
	}
	err = c.WAClient.SendAppState(appstate.BuildMute(target, action, hours))
	if err != nil {
		c.Logger.Errorf("Error changing chat's mute state: %v", err)
	} else {
		c.Logger.Infof("Mute state changed for %s to %t for %s", target, action, hours)
	}
	return err
}
